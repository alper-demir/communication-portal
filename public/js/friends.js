...
const req = async () => {
    ...
    if (response.data) {
        ...
        onlineFriends.forEach((friend) => {
            ...
            listItems += `<li><div online="1" class="${onlineClass}"></div> <a href="/chat/${friend.friend.id}"> ${friend.friend.userName} </a> </li>`
        })

        offlineFriends.forEach((friend) => {
            const onlineClass = "bg-gray-600";
            listItems += `<li><div class="${onlineClass}"></div> <a href="/chat/${friend.friend.id}"> ${friend.friend.userName} </a></li>`
        })
